...
@php
    ...
    $backUrl     = ($previousUrl && $previousUrl !== $currentUrl) ? $previousUrl : route('rekap.maintenance.ban-luar');
                        ...
                        <table class="table table-hover align-middle text-center mb-0 fs-7">
                            <thead>
                                <tr class="table-light">
                                    ...
                                    <th colspan="3" class="bg-warning bg-opacity-25 border-bottom-0 text-dark fw-bold">BAN LAMA (DILEPAS)</th>
                                    <th colspan="3" class="bg-success bg-opacity-25 border-bottom-0 text-dark fw-bold">BAN BARU (DIPASANG)</th>
                            ...
                            <tbody>
                                @foreach($invoice->details as $index => $detail)
                                    <tr>
                                        <td>{{ $index + 1 }}</td>
                                        <td class="fw-bold text-primary">{{ $detail->posisiBan->nama ?? ('Posisi '.$detail->posisi_ban_id) }}</td>

                                        <!-- Data Ban Lama -->
                                        <td class="text-uppercase fw-semibold">{{ $detail->merk_lama }}</td>
                                        <td class="text-uppercase">{{ $detail->no_seri_lama }}</td>
                                        <td>
                                            @if($detail->ritase_lama < 80)
                                                <span class="badge bg-danger fs-8" title="Ritase di bawah 80">
        ...
        <div class="modal fade" id="modalEditTanggalItem{{ $detail->id }}" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-sm">
                <div class="modal-content">
                    <form action="{{ route('rekap.maintenance.ban-luar.detail.update-tanggal', $detail->id) }}" method="POST">
                        @csrf
